...
THREE.DragControls = function ( _objects, _camera, _domElement ) {
    ...
    function setFromCamera() {
        if (_camera.isPerspectiveCamera) {
            ...
            var tmp = vector.sub(_camera.position).normalize();
            _raycaster = new THREE.Raycaster(_camera.position, tmp);
        }
        else {
            var origin = new THREE.Vector3(_mouse.x, _mouse.y, -1);
            origin = origin.unproject(_camera);
