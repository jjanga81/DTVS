...
THREE.TransformControls = function ( camera, domElement ) {
	...
	this.pointerHover = function ( pointer ) {
        ...
        if (this.camera.isPerspectiveCamera) {
            ...
            var tmp = vector.sub(this.camera.position).normalize();
            ray = new THREE.Raycaster(this.camera.position, tmp);
        }
        else {
            var origin = new THREE.Vector3(pointer.x, pointer.y, -1);
            origin = origin.unproject(this.camera);
...
THREE.TransformControlsGizmo = function () {
	...
	var arrowGeometry = new THREE.CylinderBufferGeometry( 0, 0.08, 0.2, 12, 1, false );

	var scaleHandleGeometry = new THREE.BoxBufferGeometry( 0.125, 0.125, 0.125 );

	var lineGeometry = new THREE.BufferGeometry( );
    lineGeometry.addAttribute( 'position', new THREE.Float32BufferAttribute( [ 0, 0, 0,	1, 0, 0 ], 3 ) );
    var cylinderGeometry = new THREE.CylinderBufferGeometry(0.02, 0.02, 0.95, 32, 1, false);
	...
	var gizmoTranslate = {
		X: [
            ...
            [new THREE.Mesh(cylinderGeometry, matLineRed),[0.5, 0, 0], [0, 0, - Math.PI / 2]]
		],
		Y: [
            ...
            [new THREE.Mesh(cylinderGeometry, matLineGreen), [0, 0.5, 0]]
		],
		Z: [
            ...
            [new THREE.Mesh(cylinderGeometry, matLineBlue), [0, 0, 0.5], [Math.PI / 2, 0, 0]]
		],
		XYZ: [
	...
	this.updateMatrixWorld = function () {
		...
		for ( var i = 0; i < handles.length; i ++ ) {
            ...
            var offset = 1.5;
            var v1 = new THREE.Vector3().copy(pos).project(this.camera);
            v1.y -= offset;
            var v2 = v1.unproject(this.camera);
            var v = new THREE.Vector3();
            v.subVectors(pos, v2);
            var length = v.length();
            handle.scale.set(1, 1, 1).multiplyScalar(length * this.size / 7);
